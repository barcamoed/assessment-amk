...
function PlaceCard({ place, onViewDetail }) {
  ...
  const markVisited = (place) => {
    const existing_visited = JSON.parse(localStorage.getItem('visited_place')) || []
    const updatedPlaces = places && places.length > 0 && places.map((item) => {
      if (item.id === place.id) {
        localStorage.setItem('visited_place', JSON.stringify([...existing_visited,{...item,isVisited: true }]))
  ...
  const unMarkVisited = (place) => {
    ...
    const updatedPlaces = places && places.length > 0 && places.map((item) => {
      if (item.id === place.id) {
        const filteredVisited = existing_visited?.filter(item=>item.id!==place.id)
        localStorage.setItem('visited_place', JSON.stringify([...filteredVisited]))
  ...
  return (
    <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
      <a href="#" onClick={(e) => {e.preventDefault();handleClick(place)}}>
        <img
